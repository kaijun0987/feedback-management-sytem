...
var pr = class e {
  ...
  Ae = { ParamScalar: 1, ParamEnum: 2, ParamListScalar: 4, ParamListEnum: 8, ListObject: 16, Object: 32 },
    ...
    async #r(t) {
      if (t.newTxId)
        return await this.#p(t.newTxId, 'start', async s => {
          if (s.status !== 'running')
            throw new Re(`Transaction in invalid state ${s.status} when starting a nested transaction.`);
          if (!s.transaction)
            throw new Re('Transaction missing underlying driver transaction when starting a nested transaction.');
          s.depth += 1;
          let a = this.#i(s);
          s.savepoints.push(a);
          try {
