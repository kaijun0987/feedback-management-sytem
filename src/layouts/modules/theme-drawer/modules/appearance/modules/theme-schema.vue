...
const icons: Record<Exclude<UnionKey.ThemeScheme, 'auto'>, string> = {
...
const activeThemeSchema = computed<'light' | 'dark'>(() => {
  return themeStore.themeScheme === 'dark' ? 'dark' : 'light';
});
...
<template>
  ...
  <div class="flex-col-stretch gap-16px">
    <div class="i-flex-center">
      <NTabs
        :key="activeThemeSchema"
        type="segment"
        size="small"
        class="relative w-214px"
        :value="activeThemeSchema"
        @update:value="handleSegmentChange"
      >
        <NTab v-for="(_, key) in themeSchemaRecord" :key="key" :name="key">
          <SvgIcon :icon="icons[key as keyof typeof icons]" class="h-23px text-icon-small" />
        </NTab>
      </NTabs>
    </div>
