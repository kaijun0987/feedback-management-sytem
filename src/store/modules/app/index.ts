...
export const useAppStore = defineStore(SetupStoreId.App, () => {
  ...
  /** Next language in `localeOptions` (click-to-toggle, no dropdown) */
  function toggleLocale() {
    const keys = localeOptions.map(item => item.key);
    if (keys.length < 2) {
      return;
    }

    const at = keys.indexOf(locale.value);
    const i = at === -1 ? 0 : at;
    const next = keys[(i + 1) % keys.length] as App.I18n.LangType;
